...
@pytest.mark.form
@pytest.mark.ui
@allure.title("BillPay with empty amount field")
@allure.description("""
Verify that the system enforces required field validation for the amount field.
Expected results:
- Validation message is displayed indicating the field is required
...
def test_form_submission(auth_page, payment_data_load, billpay_amount_load):
    ...
    payment_page.payment_form_fill(payment_data_load, billpay_amount_load["empty_amount"])

    # Efectuez plata
    payment_page.click_send_payment()

    # Verific ca mesajul de eroare este vizibil
    expect(auth_page.locator("#validationModel-amount-empty")).to_be_visible()
    expect(auth_page.locator("#validationModel-amount-empty")).to_have_text("The amount cannot be empty. ")
